...
contract LEFirstWallet is Ownable, ReentrancyGuard {
    ...
    function setManagementList(uint256[] memory _id, address[] memory _wallet) external onlyOwner nonReentrant {
        ...
        for(uint i=0; i<_wallet.length; i++) {
            ...
            address wallet = managementList[_id[i]].wallet;
            uint256 balance = managementList[_id[i]].balance;
            if(wallet != address(0) && balance > 0) {
    ...
    function setEntreList(uint256[] memory _id, address[] memory _wallet) external onlyOwner nonReentrant {
        require(_id.length == _wallet.length, "Arrays of different sizes");
        ...
        for(uint i=0; i<_wallet.length; i++) {
            ...
            address wallet = entreList[_id[i]].wallet;
            uint256 balance = entreList[_id[i]].balance;
            if(wallet != address(0) && balance > 0) {
               payable(wallet).transfer(balance);
            }
    ...
    function setPRList(uint256[] memory _id, address[] memory _wallet) external onlyOwner nonReentrant {
        require(_id.length == _wallet.length, "Arrays of different sizes");
        ...
        for(uint i=0; i<_wallet.length; i++) {
            ...
            address wallet = PRList[_id[i]].wallet;
            uint256 balance = PRList[_id[i]].balance;
            if(wallet != address(0) && balance > 0) {
               payable(wallet).transfer(balance);
            }
